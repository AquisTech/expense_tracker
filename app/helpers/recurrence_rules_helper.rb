  ...
  def weekday_select_bar
    content_tag :div, class: 'input-group' do
      Date::ABBR_DAYNAMES.map do |day|
        label_tag day, day, class: 'input-group-label'
      end.join.html_safe
    end
  end
    ...
    ((1..31).to_a << -1).in_groups_of(7).map do |days_of_week|
      content_tag :div, class: 'row columns' do
        days_of_week.compact.map do |day|
          unless day == -1
            content_tag :div, day, class: "small-1 columns #{'end' if (day % 7).zero?}"
          else
            content_tag :div, 'Last day of month', class: 'small-4 columns end'
          end
        end.join.html_safe
      end
    end.join.html_safe
