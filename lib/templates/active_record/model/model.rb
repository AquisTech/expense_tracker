...
<% end -%>
  ...
  # Associations
  <%- belongs_to_attrs = attributes_names.select{ |attr, value| k.ends_with?('_id') } -%>
  <% belongs_to_attrs.each do |attr, value| -%>
belongs_to :<%= attr %><%= ', polymorphic: true' if attribute_names.keys.include?(attr.gsub(/_id$/, '_type')) %>
  <% end -%>
